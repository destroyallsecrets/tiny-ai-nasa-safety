...
### Core Components
*   **Neural Core (The Brain):** `llama.cpp` or `vllm` running a quantized 2B-3B model (e.g., Phi-3, Gemma-2B).
...
*   **Deliverable:** A Python script running `tail -f /var/log/syslog`, feeding lines to a local LLM, and logging the "Proposed Action" vs "Blockchain Verdict".
...
*   **Tech:** Use eBPF to allow the AI to tune TCP congestion windows or process `nice` values in real-time.
...
| **M3: Release** | Full Integration with eBPF for network filtering (Firewall managed by AI). | High |
| **M4: Hydra** | Implementation of the Cloud Teacher (Gemini) -> Local Student (LoRA) pipeline. | Very High |

## 6. Implementation Guide for M1 (Alpha)
